...
| `view_items_in_registries`   | Full details for one or more extensions (deps, env vars, wiring).                |
| `get_docs_for_items`         | Setup-oriented docs (env, wiring, starter blocks, runner-aware install command). |
| `get_add_command_for_items`  | The add command (and variants), prefixed with the project's package runner.      |
| `get_audit_checklist`        | Project-aware post-install checklist (env vars, server wiring, starters).        |
...
<Callout type="info" title="Using Cursor?">
  ...
  Dashfy agent skill and project rules in a single installable package.
</Callout>
