...
<Callout type="info" title="Let the CLI set it up for you">
  Running [`dashfy add`](/docs/cli) performs both steps automatically — it
  registers widgets in `App.tsx`, registers the server API in your bootstrap, adds
  the required `.env` entries, and seeds a starter dashboard block.
</Callout>
